...
class AttributesDataset():         # contains labels for all variations and mapping between string names and IDs
...
class ClassificationDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_file, attributes, \
                 to_tensor_source, to_tensor_target, aug_transforms=None):
        
        self.aug_transforms = aug_transforms
        self.to_tensor_source = to_tensor_source
        self.to_tensor_target = to_tensor_target
    ...
    def get_item_from_index(self, index):
        ...
        if not os.path.isfile(os.path.join(self.data_path, self.img_names[index])):
            dic_img_name = self.img_names[index].replace('jai','JAI')     # DIC crop file names have 'JAI' string
        
        dic_image_path = os.path.join(self.data_path, dic_img_name)
        
        bf_data_path = self.data_path.replace('dic','bf') 
        bf_image_path = os.path.join(bf_data_path, self.img_names[index])
        
        bf_img = cv2.imread(bf_image_path)   
        bf_img = cv2.cvtColor(bf_img, cv2.COLOR_BGR2RGB) ## opencv reads the color channels in reverse order :(
        
        dic_img = cv2.imread(dic_image_path)   
        dic_img = cv2.cvtColor(dic_img, cv2.COLOR_BGR2RGB)
 
        img = np.concatenate((bf_img,dic_img), axis=2)   # concatenation along channels
        ...
        bf_img = self.to_tensor_source(img[0])   
        dic_img = self.to_tensor_target(img[1])
        img = torch.cat((bf_img, dic_img), 0)     # concatenate tensors along channels
